...
local function styleButton(button, color)
    ...
    button.prerender = function(b)
        ...
        if b.radiusGlyph then
            ...
            b:drawRect(cx - 5, cy - 1, 10, 2, 1, 1, 1, 1)
            if b.radiusGlyph == "+" then b:drawRect(cx - 1, cy - 5, 2, 10, 1, 1, 1, 1) end
        else
            local title = truncateText(b.renderTitle or b.fullTitle or b.title or "", b.width - 18, UIFont.Small)
...
local function drawRecipeHeroArt(panel)
    ...
    if RECIPE_HERO_ART then
        ...
        local h = panel.height + 28
        local w = math.floor(h * 1.5)
        panel:drawTextureScaled(RECIPE_HERO_ART, panel.width - w - 2, -14, w, h, .48, 1, 1, 1)
    end
end
...
local function styleDishButton(button)
    ...
    button.prerender = function(b)
        ...
        if tex then b:drawTextureScaled(tex, 9, 10, 32, 32, ready and 1 or .35, 1, 1, 1) end
        local title = truncateText(b.fullTitle or b.title, b.width - 60, UIFont.Small)
        b:drawTextCentre(title, b.width * .66, 18, 1, 1, 1, ready and 1 or .48, UIFont.Small)
        -- A status stripe communicates availability without overflowing in
        -- translated builds or competing with the dish title.
        b:drawRect(b.width - 7, 8, 3, b.height - 16, .95, color.r, color.g, color.b)
        b:updateTooltip()
    end
end
...
function CookItForMePlanUI:createChildren()
    ...
    self.strategyLabel = ISLabel:new(COL1, y + 4, 18, strategyLabel, .75, .75, .75, 1, UIFont.Small, true)
    ...
    for i, option in ipairs(self.strategyCombo.options) do
        local b = ISButton:new(0, 0, 1, 24, option.text, self, CookItForMePlanUI.onStrategyButton)
    ...
    self.radiusLabel = ISLabel:new(COL1, y + 4, 18, radiusLabel, .75, .75, .75, 1, UIFont.Small, true)
    ...
    for i, spec in ipairs({ { text = "-", delta = -1 }, { text = tostring(settings.radius), delta = 0 }, { text = "+", delta = 1 } }) do
        local b = ISButton:new(0, 0, 1, 24, spec.text, self, CookItForMePlanUI.onRadiusButton)
    ...
    self.radiusHint = ISLabel:new(radiusX + 68, y + 4, 18, getText("UI_CookItForMe_SettingsRadiusHintShort"), .55, .55, .55, 1, UIFont.Small, true)
    ...
    self.soundButton = ISButton:new(0, 0, 1, 24, getText("UI_CookItForMe_SettingsCompletionSound"), self,
    ...
    self.dishRail = ISPanel:new(8, self:titleBarHeight() + 6, self.width - 16, math.max(28, y - self:titleBarHeight() - 12))
    ...
    self.dishRail.prerender = function(panel)
        ...
        panel:drawText(truncateText(getText("UI_CookItForMe_PlanTitle"), panel.width - 20, UIFont.Small), 10, 5,
    ...
    self.summaryCard = ISPanel:new(8, y - 4, self.width - 16, 58)
    ...
    self.summaryCard.prerender = function(panel)
        ...
        panel:drawRect(0, 0, 4, panel.height, .95, SELECTED.r, SELECTED.g, SELECTED.b)
        local entry = self.entries[self.activeIndex]
        if entry and entry.plan then
            local plan = entry.plan
            local contentX = 14
            local detailWidth = panel.width - contentX - 14
            panel:drawText(truncateText(getText(Catalog.DISHES[plan.dishKey].label), detailWidth, UIFont.Medium), contentX, 20,
            ...
            panel:drawText(equipment, contentX, 46, MUTED.r, MUTED.g, MUTED.b, 1, UIFont.Small)
            local statsH = 38
            local statsY = panel.height - statsH - 8
            local statW = math.max(76, math.floor((panel.width - 42) / 2))
            local statRightX = 20 + statW
            -- Metrics are cards, not progress bars: saturated full-width
            -- fills made the centre of the UI visually heavy.
            panel:drawRect(14, statsY, statW, statsH, .62, .06, .10, .08)
            panel:drawRect(statRightX, statsY, statW, statsH, .62, .10, .075, .04)
            panel:drawRect(14, statsY, statW, 3, .95, GOOD.r, GOOD.g, GOOD.b)
            panel:drawRect(statRightX, statsY, statW, 3, .95, ACCENT.r, ACCENT.g, ACCENT.b)
            panel:drawRectBorder(14, statsY, statW, statsH, .42, GOOD.r, GOOD.g, GOOD.b)
            ...
            panel:drawText(truncateText(calories, statW - 12, UIFont.Small), 20, statsY - 4, 1, 1, 1, 1, UIFont.Small)
            panel:drawText(truncateText(hunger, statW - 12, UIFont.Small), statRightX + 6, statsY - 4, 1, 1, 1, 1, UIFont.Small)
        else
            panel:drawText(getText("UI_CookItForMe_PlanTitle"), 14, 12, .95, .48, .38, 1, UIFont.Small)
            panel:drawText(failText(entry and entry.failKey), 14, 34, .95, .75, .70, 1, UIFont.Small)
        end
    end
    self:addChild(self.summaryCard)

    self.content = NIScrollView:new(8, y + 60, self.width - 16, self.height - y - 118)
    ...
    self.details.prerender = function(panel)
        ...
        local lineH = math.max(24, fontH + 4)
        for _, line in ipairs(self.lines) do
            ...
            local rowH = math.max(lineH + 8, #rows * lineH + 8)
            if line.kind == "header" then
                ...
                panel:drawRect(6, top - 4, panel.width - 24, rowH, .62, .08, .08, .08)
                panel:drawRect(6, top - 4, 4, rowH, .95, ACCENT.r, ACCENT.g, ACCENT.b)
            elseif line.kind == "item" then
                panel:drawRect(4, top - 4, panel.width - 22, rowH, .38, .12, .12, .12)
                panel:drawRect(4, top - 4, 2, rowH, .44, MUTED.r, MUTED.g, MUTED.b)
                if line.frozen then panel:drawRect(4, top - 4, 3, rowH, .88, .24, .60, .92) end
            elseif line.kind == "spice" then
                panel:drawRect(4, top - 4, panel.width - 22, rowH, .46, .11, .095, .055)
                panel:drawRect(4, top - 4, 3, rowH, .88, ACCENT.r, ACCENT.g, ACCENT.b)
            end
            if line.tex then
                panel:drawTextureScaled(line.tex, 8, top + math.floor((rowH - 24) / 2) - 4, 24, 24, 1, 1, 1, 1)
            end
        ...
        panel:setHeight(top + 10)
        self.content:setScrollHeight(top + 10)
...
function CookItForMePlanUI:prerender()
    ...
    local pad, railW, gap = 12, 188, 12
    local railX, railY = pad, titleH + pad
    local mainX = railX + railW + gap
    local mainW = math.max(220, self.width - mainX - pad)
    local btnY = self.height - self:resizeWidgetHeight() - 44
    ...
    local railH = math.min(math.max(80, btnY - railY - 8), 62 + #self.tabButtons * 54)
    ...
    railTitle = truncateText(railTitle, railW - 24, UIFont.Small)
    self:drawText(railTitle, railX + 12, railY + 10, ACCENT.r, ACCENT.g, ACCENT.b, 1, UIFont.Small)
    for i, button in ipairs(self.tabButtons) do
        ...
        button:setY(railY + 50 + (i - 1) * 54)
        button:setWidth(railW - 16)
        button:setHeight(48)
    end

    self:persistGeometry()
    local compactHeight = self.height < 500
    ...
    local heroY, heroH = railY, compactHeight and 120 or 132
    self.summaryCard:setX(mainX); self.summaryCard:setY(heroY)
    self.summaryCard:setWidth(mainW); self.summaryCard:setHeight(heroH)

    local settingsY = heroY + heroH + 10
    -- At the minimum supported width, the radius and sound controls must not
    -- fight for one row.  Reflow the sound control below them instead.
    local compactSettings = mainW < 470
    local settingsH
    if compactSettings then
        settingsH = compactHeight and 150 or 170
    else
        settingsH = compactHeight and 112 or 124
    end
    drawNeatSurface(self, "media/ui/NeatUI/DefaultPanel/ContentPanel_BG.png", mainX, settingsY, mainW, settingsH,
        .78, .08, .08, .08)
    self:drawText(truncateText(getText("UI_CookItForMe_Settings"), mainW - 24, UIFont.Small), mainX + 12, settingsY + 8,
    ...
    local choiceX, choiceY = mainX + 12, settingsY + (compactHeight and 44 or 48)
    local choiceW = math.floor((mainW - 32) / #self.strategyButtons)
    for i, b in ipairs(self.strategyButtons) do
        b:setX(choiceX + (i - 1) * (choiceW + 4)); b:setY(choiceY)
        b:setWidth(choiceW); b:setHeight(28)
    end
    self.radiusSpin:setVisible(false); self.completionSoundTick:setVisible(false)
    local radiusLabel = getText("UI_CookItForMe_SettingsRadius")
    local radiusX = mainX + math.max(100, getTextManager():MeasureStringX(UIFont.Small, radiusLabel) + 24)
    for i, b in ipairs(self.radiusButtons) do
        b:setX(radiusX); b:setY(settingsY + (compactHeight and 80 or 86))
        b:setWidth(i == 2 and 42 or 26); b:setHeight(26)
        radiusX = radiusX + b.width + 3
    end
    if compactSettings then
        self.soundButton:setX(mainX + 12); self.soundButton:setY(settingsY + (compactHeight and 116 or 122))
        self.soundButton:setWidth(mainW - 24)
    else
        self.soundButton:setX(radiusX + 12); self.soundButton:setY(settingsY + (compactHeight and 80 or 86))
        self.soundButton:setWidth(math.max(100, mainX + mainW - self.soundButton.x - 12))
    end
    self.soundButton:setHeight(26)
    self:drawText(truncateText(radiusLabel, math.max(20, self.radiusButtons[1].x - mainX - 20), UIFont.Small), mainX + 12,
        settingsY + (compactHeight and 72 or 76), .82, .82, .82, 1, UIFont.Small)
    self.radiusSpin:setWidth(62)
    self.strategyLabel:setVisible(false); self.radiusLabel:setVisible(false); self.radiusHint:setVisible(false)

    self.content:setX(mainX); self.content:setY(settingsY + settingsH + 10)
    self.content:setWidth(mainW)
    self.content:setHeight(math.max(32, btnY - self.content.y - 10))
    ...
    self.cookButton:setX(mainX); self.cookButton:setWidth(math.max(120, mainW * .62)); self.cookButton:setY(btnY)
    self.closeButton:setX(mainX + self.cookButton.width + 8); self.closeButton:setWidth(math.max(88, mainW - self.cookButton.width - 8)); self.closeButton:setY(btnY)
...
function CookItForMePlanUI:new(player, entries, activeIndex)
    ...
    -- Legacy dimensions were tuned for a plain text dump and create a huge
    -- dead area with the card layout.  Start compact; the player can still
    -- enlarge the resizable window when they genuinely need more room.
    local w = math.min(sw, math.max(math.min(sw, 560), tonumber(settings.panelW) or 980))
    local h = math.min(sh, math.max(math.min(sh, 420), tonumber(settings.panelH) or 560))
